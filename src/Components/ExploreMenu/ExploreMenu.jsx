...
const ExploreMenu = ({category, setCategory}) => {
  ...
  return (
    <div className='mt-10 text-center'>
      ...
      <div
      className='flex gap-8 justify-center mt-5 overflow-x-auto overflow-hidden'>
        {menu_list.map((item, index) => {
          return (
            <div
            onClick={() => setCategory(prev=>prev===item.menu_name ? 'all' : item.menu_name)} 
            className='text-center' key={index}>
              <img className={`w-[7.5vh] min-w-24 cursor-pointer ${
      category === item.menu_name ? 'border-4 border-green-500 p-1 rounded-full' : ''
